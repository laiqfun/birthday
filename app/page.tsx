...
export default function Home() {
  ...
  const window_data = [
    ...
    },
    {
      ...
      w: 300,
      h: 140,
      dom: (<div className={"flex-1 flex items-center justify-center"}>
        <input className={"input"} value={text} onChange={(e) => setText(e.target.value)}
               style={{borderRadius: "5px 0 0 5px"}}/>
        <button className={"button"} style={{borderRadius: "0 5px 5px 0 "}}>提交</button>
      </div>)
    },
    {
      title: "获取『追·卡券』",
      w: 320,
      h: 240,
      dom: (<div className={"flex-1 flex flex-col"}>
    ...
    <main className={"flex flex-col w-screen h-screen"}>
      ...
      <div className={"flex items-center h-14 pr-2"}>
        ...
        <div className={"w-80 flex items-center"}>
          <div className={"flex-1"}></div>
          <div className={"px-1"}>
            <button className={"btn"} onClick={() => openWindow(1)}>兑换码</button>
          </div>
          <div className={"px-1"}>
            <div id={"ticket"}>
              <div><Image fill src={"/vercel.svg"} alt={"追·卡券"}/></div>
              <span>7</span>
        ...
        <div id={"banner"}>
          <div>
            <div id={"banner-hero"} className={"absolute"}>
              <Image width={100} height={100} src={"/vercel.svg"} alt={"五星"}/>
            </div>
          </div>
